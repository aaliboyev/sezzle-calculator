...
- **Guided mode explains recognized patterns.** `src/engine/guides.ts` matches the field's MathJSON against templates with number slots: digits stay editable while structure holds, and a panel below the result shows worked steps (and for some patterns a reactive SVG — the pythagoras triangle redraws from the actual legs). Breaking the structure simply dismisses the guide.

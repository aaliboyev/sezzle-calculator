...
- **A formula catalog drives discovery.** `src/engine/formulas.ts` holds ~40 computable formulas in seven color-coded categories (geometry, money, statistics, roots & powers, curiosities, science, edge cases), browsable in a library sheet; a seeded random sample floats around the free space as drifting cards. Every catalog entry is machine-checked against the translator in unit tests.
- **Guided mode explains recognized patterns.** `src/engine/guides.ts` matches the field's MathJSON against 36 templates with number slots: digits stay editable while structure holds, and a panel below the result shows worked steps computed from the live values. Seventeen patterns carry reactive SVG diagrams (a triangle that redraws from the actual legs, a parabola whose crossings follow the discriminant's sign, convergence dots for e, cancelling factor tiles for exponent laws) — either declarative shape specs rendered by one component or bespoke components under a shared contract. Breaking the structure simply dismisses the guide; deliberately generic shapes (a bare `a·b` or `a/b`) are left unguided rather than mislabeled.
...
- **Single-binary deploy.** `go build -tags embed` serves the built frontend from the Go binary; dev builds skip the embed and Vite proxies `/api`. The server sets `nosniff`, a same-origin CSP (inline styles and `data:` fonts allowed — MathLive's rendered markup requires both), and `Referrer-Policy`; directory listings are suppressed; the Docker image runs as nonroot.

...
// MathLive fires input events on keys that change nothing (Enter included);
// only a real value change may clear the outcome.
let lastInputLatex = ''
  ...
  persist(
    (set, get) => ({
      ...
      syncGuide: (latex) => {
        ...
        if (next) {
          set({ guide: next, guideStale: false })
        } else if (!latex.trim()) {
          if (prev) set({ guide: null, guideStale: false })
        } else if (prev) {
      ...
      handleInput: (latex) => {
        if (latex === lastInputLatex) return
        lastInputLatex = latex
        set({ outcome: null })
        get().syncGuide(latex)
      },
      ...
      pressKey: (label, insert) => {
        const { field, submit, clearOutcome, handleInput } = get()
        if (!field) return
        if (label === '=') {
          void submit()
        } else {
          if (label === 'AC') field.value = ''
          else if (label === '⌫') field.executeCommand('deleteBackward')
          else field.executeCommand(['insert', insert ?? label])
          clearOutcome()
